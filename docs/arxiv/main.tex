...
\begin{tikzpicture}[
    node distance=0.3cm,
    db/.style={draw, rounded corners, minimum width=1.5cm, minimum height=0.5cm, font=\scriptsize, fill=green!10},
    proc/.style={draw, rounded corners, minimum width=2.5cm, minimum height=0.5cm, font=\scriptsize, fill=blue!8},
    arr/.style={-{Stealth[length=1.5mm]}, thick},
]
    \node[db] (oa) {OpenAlex};
    \node[db, below=0.15cm of oa] (epm) {Europe PMC};
    \node[db, below=0.15cm of epm] (pm) {PubMed};
    \node[db, below=0.15cm of pm] (s2) {Sem. Scholar};
    \node[db, below=0.15cm of s2] (ar) {arXiv};
    \node[db, below=0.15cm of ar] (cr) {CrossRef};

    \node[proc, right=1.2cm of pm] (dedup) {DOI Dedup};
    \node[proc, right=0.5cm of dedup] (kw) {Keyword Filter};
    \node[proc, right=0.5cm of kw] (llm) {LLM Selection};
    \node[proc, right=0.5cm of llm] (upw) {Unpaywall};

    \draw[arr] (oa.east) -- (dedup.west |- oa);
    \draw[arr] (epm.east) -- (dedup.west |- epm);
    \draw[arr] (pm.east) -- (dedup.west);
    \draw[arr] (s2.east) -- (dedup.west |- s2);
    \draw[arr] (ar.east) -- (dedup.west |- ar);
    \draw[arr] (cr.east) -- (dedup.west |- cr);
    \draw[arr] (dedup) -- (kw);
    \draw[arr] (kw) -- (llm);
    \draw[arr] (llm) -- (upw);

    \node[font=\tiny, above=0.15cm of dedup] {50+ papers};
    \node[font=\tiny, above=0.15cm of kw] {30+};
    \node[font=\tiny, above=0.15cm of llm] {12};
    \node[font=\tiny, above=0.15cm of upw] {68\% OA};
\end{tikzpicture}
\caption{Search and filtering pipeline. Six databases are queried in parallel (with domain-aware source selection), followed by three filtering stages and open-access enrichment.}
\label{fig:search}
\end{figure}
